...
class MethodChannelAMapFlutterMap implements AMapFlutterPlatform {
  ...
  Widget buildView(
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      ...
      // [本地补丁 2026-09-25] 原实现 AndroidView（纹理/TLHC 模式）在部分
      // 设备上创建视图时抛 PlatformException NullPointerException
      // （TextureAndroidViewController._sendCreateMessage → 引擎
      // PlatformViewsController NPE），地图不显示。
      // 改用 Hybrid Composition（SurfaceAndroidView）绕开纹理路径，
      // 这是老版官方 amap flutter 插件长期使用的成熟模式。
